...
export function Header({ data, locale, languageLinks, messages }: HeaderProps) {
  ...
  return (
    <>
      <header className={cn(siteHeader, siteHeader.bod("scrolled", pastHero), siteHeader.bod("menu-open", menuOpen))}>
        ...
        <a className={siteHeader.el("mark")} href={data.homeHref} aria-label={messages.header.home}>
          <img className={siteHeader.el("mark-image")} src={data.logo} alt="" />
        </a>
        <div className={siteHeader.el("actions")}>
          <nav className={localeSwitch} aria-label={messages.header.language}>
            <a className={cn(localeSwitch.el("option"), localeSwitch.el("option").bod("active", locale === "en"))} href={languageLinks.en} lang="en" aria-current={locale === "en" ? "page" : undefined}>EN</a>
            <span>/</span>
            <a className={cn(localeSwitch.el("option"), localeSwitch.el("option").bod("active", locale === "ru"))} href={languageLinks.ru} lang="ru" aria-current={locale === "ru" ? "page" : undefined}>RU</a>
          </nav>
          <button
            className={menuTrigger}
            type="button"
            aria-label={menuOpen ? messages.header.close : messages.header.menu}
      ...
      <div className={cn(menuOverlay, menuOverlay.bod("open", menuOpen))} aria-hidden={!menuOpen}>
        <div className={menuOverlay.el("panel")}>
          <nav className={menuOverlay.el("navigation")} aria-label={messages.header.navigation}>
